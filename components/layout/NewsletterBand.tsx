          ...
          {(config.socials ?? []).map((social) => (
            <li key={social._uid}>
              ...
              <SmartLink
                ...
                className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-brand text-white transition-opacity hover:opacity-85 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand"
              >
...
function SocialIcon({ platform }: { platform: SocialLinkBlok["platform"] }) {
  ...
  switch (platform) {
    case "linkedin":
      return (
        <svg {...common} className="h-[1.15rem] w-[1.15rem]">
          <path
            fillRule="evenodd"
